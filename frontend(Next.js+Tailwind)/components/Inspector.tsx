        ...
        <div className="w-80 h-full bg-[#0a0a0a] border-l border-[#1a1a1a] flex flex-col text-sm">
            ...
            <div className="p-4 space-y-4 overflow-y-auto flex-1 custom-scrollbar">
                <section>
                   ...
                   <p className="text-xs mt-2 text-white/60 italic font-mono">{feasibilityScore.toFixed(1)}% - {feasibilityScore > 80 ? 'Ready' : 'Analyzing...'}</p>
                </section>
                        ...
                        >
                            <div className="flex items-center justify-between">
                                <span className={`text-[10px] font-bold uppercase ${
                                    log.agent.includes('Director') ? 'text-red-500' : 'text-blue-500'
                                }`}>
                                    {log.agent}
                                </span>
                                <span className="text-[8px] text-gray-600 uppercase">{log.timestamp}</span>
                            </div>

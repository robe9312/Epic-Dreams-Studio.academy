--- frontend/store/useTimelineStore.ts (原始)
...
export const useTimelineStore = create<TimelineState>()(
  ...
  updateClipContent: (track, clipId, content) => {
    const state = get();
  ...
  updateClip: (track, clipId, partial, skipHistory = false) => {
    const state = get();
    const clip = state.tracks[track].find(c => c.id === clipId);
  ...
  moveClip: (fromTrack, toTrack, clipId, newStartTime) => {
    const state = get();
  ...
  updateTrackConfig: (trackId, config) => {
    set((s) => ({
      trackConfigs: {
        ...s.trackConfigs,
...
+++ frontend/store/useTimelineStore.ts (修改后)
import { create } from 'zustand';
import { subscribeWithSelector } from 'zustand/middleware';
